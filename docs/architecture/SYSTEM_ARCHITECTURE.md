...
Repository scaffold only: this documentation set, workspace layout, `packages/shared` domain enums/types, a minimal NestJS API skeleton (health endpoints + empty module directories), placeholder Expo and Next.js apps clearly marked as scaffolds, docker-compose for local dependencies, and CI. No product features are implemented yet — see `docs/development/ROADMAP.md` for build order.
